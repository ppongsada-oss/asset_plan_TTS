...
import { FileText, Calendar, Clock, ChevronRight, Loader2, Filter, LayoutGrid } from "lucide-react";
    ...
    <div className="mt-8">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {jobs.map((job) => {
            const isClosed = job.status === "CLOSED" || new Date(job.end_date) < new Date();
            ...
            return (
              <Link href={`/site-plan/${job.id}`} key={job.id} className="block group">
                <div className={`bg-white rounded-2xl border p-6 transition-all shadow-sm group-hover:shadow-md ${
                  isClosed ? "border-slate-200 opacity-75" : "border-indigo-100 group-hover:border-indigo-300"
                }`}>
                  ...
                  <h3 className="text-xl font-black text-indigo-700 leading-tight mb-1 group-hover:text-indigo-600 transition-colors">
                  ...
                  <div className="space-y-2 mt-4 text-sm text-slate-600">
                    ...
                    <div className="flex items-center gap-2">
                      <Clock size={16} className={isClosed ? "text-slate-400" : "text-rose-400"} />
                      <span>Deadline: <span className="font-medium">{job.end_date}</span></span>
                    </div>
                  </div>

                  <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-sm font-medium">
                    <span className={isClosed ? "text-slate-500" : "text-indigo-600"}>
                      {isClosed || job.status === "APPROVED" || job.status === "SUBMITTED" ? "ดูรายละเอียด" : "เริ่มวางแผน"}
                    </span>
                    <ChevronRight size={18} className={isClosed ? "text-slate-400" : "text-indigo-400"} />
                  </div>
                </div>
              </Link>
            );

...
description: Manages the lifecycle of files and their dependencies in index_files.json.
---

# File Index Manager

## Triggers & The Many-to-Many Backlink Rule
You must execute your duties on `index_files.json` ONLY under these conditions:
...
## Pre-Analysis Role
Before the Coder or Editor touches a file, use `grep_search` against this index to ensure you understand all `backlinks` that might be affected by the upcoming code change.
